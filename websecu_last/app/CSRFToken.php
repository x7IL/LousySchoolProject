...
function getUserRoleFromCSRFToken($token)
{
    ...
    $decodedToken = decodeJSONWithPrivateKey($token,dirname( dirname( __FILE__ ) ). DIRECTORY_SEPARATOR ."private/token.key",'');
...
function generateCSRFToken($userId, $username, $role)
{
    ...
    $base64 = encodeJSONWithPublicKey($json,dirname( dirname( __FILE__ ) ). DIRECTORY_SEPARATOR ."private/token.crt");
